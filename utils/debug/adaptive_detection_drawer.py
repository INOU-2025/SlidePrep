...
from utils.detection.template_matching import DetectionStrategy, draw_border_overlay, draw_contours_with_strategy
...
class AdaptiveDetectionDrawer(BaseDrawer):
    ...
    def draw(self, image: np.ndarray, results: Any = None, metadata: Any = None) -> Optional[np.ndarray]:
        ...
        for orientation, (mask, contours) in detections.items():
            if contours and strategies.get(orientation):
                ...
                if detector:
                    ...
                    template_shape = (config['thickness'], config['template_length'])
        ...
        if self.show_strategy_info:
            base = self._add_strategy_overlay(base, strategies, detections, detector)
    ...
    def _add_strategy_overlay(self, image: np.ndarray, strategies: Dict[str, DetectionStrategy], 
        ...
        for orientation in ['horizontal', 'vertical']:
            ...
            if strategy:
                ...
                valid_count = len([c for c in contours if cv2.contourArea(c) >= min_area])
                
                # Strategy-specific color
                if strategy == DetectionStrategy.GENERAL:
                    strategy_color = (0, 255, 0) if orientation == 'vertical' else (255, 0, 0)
                elif strategy == DetectionStrategy.THICK_BORDER:
                    strategy_color = (0, 200, 0) if orientation == 'vertical' else (200, 0, 0)
                else:  # THIN_BORDER
                    strategy_color = (0, 150, 0) if orientation == 'vertical' else (150, 0, 0)
    ...
    def _add_cache_overlay(self, image: np.ndarray, cache_stats: Dict[str, int]) -> np.ndarray:
        ...
        # Template cache
        y_offset += 15
        template_hits = cache_stats.get('template_hits', 0)
        template_misses = cache_stats.get('template_misses', 0)
        ...
        # Preprocessing cache
        y_offset += 15
        preprocessing_hits = cache_stats.get('preprocessing_hits', 0)
        preprocessing_misses = cache_stats.get('preprocessing_misses', 0)
